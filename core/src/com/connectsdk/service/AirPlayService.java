...
public class AirPlayService extends DeviceService implements MediaPlayer, MediaControl {
    ...
    public void getPositionAndDuration(final PositionAndDurationListener listener) {
        getPlaybackPosition(new PlaybackPositionListener() {

            @Override
            public void onGetPlaybackPositionSuccess(long duration, long position) {
                Util.postSuccess(listener, new Pair<>(duration, position));
            }

            @Override
            public void onGetPlaybackPositionFailed(ServiceCommandError error) {
                Util.postError(listener, new ServiceCommandError(0, "Unable to get position and duration", null));
            }
        });
    }
        ...
        getPlaybackInfo(new ResponseListener<Object>() {
            ...
            public void onSuccess(Object object) {
                ...
                try {
                    ...
                    text = text.replace("\n", "");
                    text = text.replace("\t", "");
                    for (String s : splitEqually(text, 50)) {
                        Log.d(Util.T, "getPlayState() failed: response=" + s);
                    }
                    // -anton
    ...
    public void playVideo(final String url, String mimeType, String title,
        ...
        builder.putReal("Start-Position", startPosition);
    ...
    public void playMedia(String url, String mimeType, String title,
        ...
        if (mimeType.contains("image")) {
            ...
            playVideo(url, mimeType, title, description, iconSrc, shouldLoop, startPosition, listener);
        }
    }
    ...
    public void playMedia(MediaInfo mediaInfo, boolean shouldLoop, LaunchListener listener) {
        ...
        playMedia(mediaUrl, mimeType, title, desc, iconSrc, shouldLoop, startPosition, listener);
    }
        ...
        Util.runInBackground(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if("/stop".equals(serviceCommand.getTarget())) {
                        // don't fire onPairingRequired for |stop| command
                        checkAuth = false;
                    }
                    ...
                    if (code == HttpURLConnection.HTTP_OK) {
                        ...
                        if(checkAuth) {
                            ...
                            Log.d(Util.T, "airplay: skip failed auth: target=" + serviceCommand.getTarget());
    ...
    public void connect() {
        ...
        LOGD("connect: new session id: " + mSessionId);
